...
module DiaRoot::DiaTests {
    ...
    fun test_set_value(root_acc: signer, account: signer) {
        ...
        Dia::set_price_value<BTC, USDT>(&account, value, timestamp);

        // Get value and match.
        let (v, ts) = Dia::get_price_value<BTC, USDT>(acc_addr);
    ...
    fun test_rewrite_value(root_acc: signer, account: signer) {
        ...
        Dia::set_price_value<BTC, USDT>(&account, new_value, new_timestamp);

        // Get new values and match.
        let (new_v, new_t) = Dia::get_price_value<BTC, USDT>(acc_addr);
    ...
    fun test_diff_curr(root_acc: signer, account: signer) {
        ...
        // Set value for transaction sender.
        Dia::set_price_value<BTC, USDT>(&account, btc_value, btc_timestamp);
        ...
        // Set value for transaction sender.
        Dia::set_price_value<DIA, USDT>(&account, dia_value, dia_timestamp);

        let (btc_v, btc_t) = Dia::get_price_value<BTC, USDT>(acc_addr);
        let (dia_v, dia_t) = Dia::get_price_value<DIA, USDT>(acc_addr);
